...
root.title("casa de la hamburguesa")
def orden():
    lista=""
    if (queso.get()):
        lista+="con queso"
...
queso=IntVar()
lechuga=IntVar()
#imagen = PhotoImage(file="hamburguesa.JPEG")
#Label(root, image=imagen).pack(side=LEFT)
frame=Frame(root)
frame.pack(side=RIGHT)
frame.config(bg="goldenrod3")
Label(frame, text="¿Cómo quieres tu hambrguesa?",bg="goldenrod3", font="Curier 15").pack(anchor="w")
